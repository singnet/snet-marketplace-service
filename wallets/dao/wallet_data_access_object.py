...
class WalletDAO:
    ...
    def insert_wallet_details(self, username, address, type, status):
        try:
            ...
            default_wallet = self.repo.execute(default_wallet_query,
                                               [username, 1])
    ...
    def set_default_wallet(self, username, address):
        try:
            ...
            update_default_query_response = self.repo.execute(
                "UPDATE user_wallet SET is_default = %s WHERE username = %s AND address = %s",
            ...
            if update_default_query_response[0] == 1:
                ...
                self.repo.commit_transaction()
            raise Exception(
                "Unable to set default wallet as %s for username %s", address,
                username)
