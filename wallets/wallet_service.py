...
class WalletService:
    ...
    def open_channel_by_third_party(self, order_id, sender, sender_private_key, group_id, amount, currency, recipient):
        ...
        agi_tokens = self.__calculate_agi_tokens(
            amount=amount, currency=currency)
        r, s, v, signature = self.__generate_signature_details(recipient=recipient, group_id=group_id,
                                                               agi_tokens=agi_tokens, expiration=expiration,
                                                               message_nonce=current_block_no,
                                                               signer_key=sender_private_key)

        positional_inputs = (
            sender, SIGNER_ADDRESS, recipient, group_id, agi_tokens, expiration, current_block_no, v, r, s)
