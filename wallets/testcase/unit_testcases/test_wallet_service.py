...
class TestWalletService(unittest.TestCase):
    ...
    def test_create_wallet(self, mock_register_wallet, mock_create_account,
                           mock_report_slack):
    ...
    @patch("common.utils.Utils.report_slack")
    @patch(
        "wallets.dao.wallet_data_access_object.WalletDAO.insert_wallet_details"
    )
    def test_register_wallet(self, mock_insert_wallet_details,
                             mock_report_slack):
        ...
        try:
            self.obj_wallet_manager.register_wallet(username="dummy",
