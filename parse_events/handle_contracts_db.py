...
class HandleContractsDB:
    ...
    def _create_channel(self, q_dta, conn):
        upsrt_mpe_chnl = (
            "INSERT INTO mpe_channel (channel_id, sender, recipient, groupId, balance_in_cogs, pending, nonce, "
    ...
    def _del_srvc_dpndts(self, org_id, service_id, conn):
        ...
        print(
            "_del_srvc_dpndts::del_srvc_grps: ",
            del_srvc_grps_count,
            "|del_srvc_endpts: ",
            del_srvc_endpts_count,
        )
    ...
    def _create_or_updt_srvc_mdata(
        ...
        upsrt_srvc_mdata_params = [
            srvc_rw_id,
            org_id,
            service_id,
            ipfs_data["display_name"],
            ipfs_data["model_ipfs_hash"],
            desc,
            url,
            json_str,
            ipfs_data["encoding"],
            ipfs_data["service_type"],
            ipfs_data["mpe_address"],
            assets_hash,
            assets_url_str,
            '{"rating": 0.0 , "total_users_rated": 0 }',
            dt.utcnow(),
            dt.utcnow(),
            srvc_rw_id,
            ipfs_data["display_name"],
    ...
    def _push_asset_to_s3_using_hash(self, hash, org_id, service_id):
        ...
        new_url = self.s3_util.push_io_bytes_to_s3(
            ASSETS_PREFIX + "/" + org_id + "/" + service_id + "/" + filename,
            ASSETS_BUCKET_NAME,
            io_bytes,
        )
        return new_url
    ...
    def _comapre_assets_and_push_to_s3(
        ...
        for new_asset_type, new_asset_hash in new_assets_hash.items():
            ...
            if isinstance(new_asset_hash, list):
                ...
                logger.info(
                    "unknown type assets for org_id %s  service_id %s",
                    org_id,
                    service_id,
                )
    ...
    def _get_new_assets_url(self, org_id, service_id, new_ipfs_data):
        ...
        assets_url_mapping = self._comapre_assets_and_push_to_s3(
            existing_assets_hash,
            new_assets_hash,
            existing_assets_url,
            org_id,
