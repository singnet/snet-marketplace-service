...
class HandleContracts:
    def __init__(self, netId):
        ...
        self.db_obj = HandleContractsDB(err_obj=self.err_obj, netId=netId)

    def _init_w3(self):
        try:
            self.web3_pvdr = NETWORKS[self.netId]['ws_provider']
    ...
    def _rd_cntrct_addr(self, path, key):
        ...
        return Web3.toChecksumAddress(contract[str(self.netId)][key])
    ...
    def __handle_events(self, evt_data):
        ...
        if event in MPE_EVTS:
            self.mpe_cntrct = self._loadContract(MPE_CNTRCT_PATH) if self.mpe_cntrct == None else self.mpe_cntrct
            self.mpe_cntrct_addr = self._rd_cntrct_addr(MPE_ADDR_PATH,
                                                        'address') if self.mpe_cntrct_addr == None else self.mpe_cntrct_addr

            inst_cntrct = self._contrct_inst(contrct=self.mpe_cntrct, addr=self.mpe_cntrct_addr)
            if event == 'ChannelOpen':
                ...
                channel_data = inst_cntrct.functions.channels(channel_id).call()
                group_id = base64.b64encode(channel_data[4]).decode('utf8')
                self.db_obj.update_channel(channel_id=channel_id, group_id=group_id, channel_data=channel_data)
        elif event in REG_EVTS:
            self.reg_cntrct = self._loadContract(REG_CNTRCT_PATH) if self.reg_cntrct == None else self.reg_cntrct
            self.reg_cntrct_addr = self._rd_cntrct_addr(REG_ADDR_PATH,
                                                        'address') if self.reg_cntrct_addr == None else self.reg_cntrct_addr
            inst_cntrct = self._contrct_inst(contrct=self.reg_cntrct, addr=self.reg_cntrct_addr)
            ...
            if event in ['OrganizationCreated', 'OrganizationModified']:
                org_data = inst_cntrct.functions.getOrganizationById(org_id_hex).call()
    ...
    def _process_events(self, evts_data, type):
        for evt_data in evts_data:
            try:
                print('process_events::row_id: ', evt_data['row_id'], '|', type, '|', evt_data['block_no'])
                handled = self.__handle_events(json.loads(evt_data['json_str']))
                if handled:
                    self.db_obj.updt_raw_evts(evt_data['row_id'], type, err_cd=0, err_msg='')
            except Exception as e:
                err_msg = self.err_obj.log_err_msg(err=e, fn_nme='_process_events')
                print('writing error for row_id: ', evt_data['row_id'])
                self.db_obj.updt_raw_evts(evt_data['row_id'], type, err_cd=1, err_msg=err_msg)
