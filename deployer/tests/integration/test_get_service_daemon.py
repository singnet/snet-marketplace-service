...
class TestGetServiceDaemonHandler:
    ...
    def test_get_service_daemon_success(
        ...
        order1 = test_data_factory.create_order(
            order_id="test-order-full-001", daemon_id=daemon.id, status=OrderStatus.SUCCESS
        )
        order2 = test_data_factory.create_order(
            order_id="test-order-full-002", daemon_id=daemon.id, status=OrderStatus.PROCESSING
        )
        order3 = test_data_factory.create_order(
            order_id="test-order-full-003", daemon_id=daemon.id, status=OrderStatus.FAILED
        )
        ...
        assert not daemon_data["servicePublished"]
    ...
    def test_get_service_daemon_with_config(
        self, authorized_event, lambda_context, db_session, test_data_factory, daemon_repo
    ):
    ...
    def test_get_service_daemon_forbidden_for_other_user(
        self, get_service_daemon_event, lambda_context, db_session, test_data_factory, daemon_repo
    ):
