...
class TestGetTransactionsHandler:
    ...
    def test_get_transactions_by_order_id(
        ...
        order = test_data_factory.create_order(
            order_id="test-order-get-tx-001", daemon_id=daemon.id, status=OrderStatus.SUCCESS
        )
    ...
    def test_get_transactions_by_daemon_id(
        ...
        order1 = test_data_factory.create_order(
            order_id="test-order-get-tx-002", daemon_id=daemon.id, status=OrderStatus.SUCCESS
        )
        order2 = test_data_factory.create_order(
            order_id="test-order-get-tx-003", daemon_id=daemon.id, status=OrderStatus.PROCESSING
        )
