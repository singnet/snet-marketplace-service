...
class TestGetOrderHandler:
    ...
    def test_get_order_success(
        ...
        order = test_data_factory.create_order(
            order_id="test-order-001", daemon_id=daemon.id, status=OrderStatus.SUCCESS
        )
    ...
    def test_get_order_with_transactions(
        ...
        # Create order
        order = test_data_factory.create_order(
            order_id="test-order-002", daemon_id=daemon.id, status=OrderStatus.PROCESSING
        )
        order_repo.create_order(db_session, order)
    ...
    def test_get_order_fails_for_wrong_user(
        ...
        # Create order for this daemon
        order = test_data_factory.create_order(
            order_id="test-order-other", daemon_id=daemon.id, status=OrderStatus.SUCCESS
        )
        order_repo.create_order(db_session, order)
        db_session.commit()
