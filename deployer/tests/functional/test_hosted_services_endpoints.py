...
from deployer.application.handlers.hosted_services_handlers import get_hosted_service, check_github_repository, \
    get_hosted_service_logs, update_hosted_service_status
...
from deployer.tests.functional.utils import generate_request_event, validate_response_ok, create_common_queue_event

logger = get_logger(__name__)


class TestHostedServicesEndpoints:
    def test_get_hosted_service_ok(self, test_hosted_services_service, test_auth_service, add_test_daemon_and_service, test_org_id, test_service_id, test_hosted_service_id):
    ...
    def test_get_hosted_service_logs_ok(self, test_hosted_services_service, test_auth_service, add_test_daemon_and_service, test_hosted_service_id):
        event = generate_request_event(path_parameters = {"hostedServiceId": test_hosted_service_id})

        response = get_hosted_service_logs(event, None, test_hosted_services_service, test_auth_service)
        _, data = validate_response_ok(response)

        assert data == ["log1", "log2", "log3"]

    def test_update_hosted_service_status_ok(self, test_hosted_services_service, test_session_factory, add_test_daemon_and_service, test_org_id, test_service_id, test_hosted_service_id):
        ...
        with session_scope(test_session_factory) as session:
            hosted_service = HostedServiceRepository.get_hosted_service(session, test_hosted_service_id)

        assert hosted_service.status.value == test_status
        assert hosted_service.last_commit_url == test_hosted_services_service._github_api_client.make_commit_url(
