    ...
    def check_access(
        ...
        with session_scope(self.session_factory) as session:
            if daemon_id is not None:
                daemon = DaemonRepository().get_daemon_by_account_and_daemon(session, account_id, daemon_id)
            else:
                daemon = DaemonRepository().get_daemon_by_account_and_order(session, account_id, order_id)
