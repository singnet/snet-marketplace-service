...
class TestPythonBoilerplate(unittest.TestCase):
    def test_boilerplate_content(self):
        for content_file in PYTHON_BOILERPLATE_TEMPLATE:
            context = StubsGeneratorService._prepare_data(
                PYTHON_BOILERPLATE_TEMPLATE[content_file]["content"],
                "test_org_id",
                "test_service_id",
            )
            ...
            if content_file == "readme":
                assert (
                    context
                    == "Follow below instructions to setup dependencies :1.Create virtual environment   "
            ...
            if content_file == "config":
                assert (
                    context
                    == f'PRIVATE_KEY = "<your wallet\'s private key>"ETH_RPC_ENDPOINT = "https://{settings.network.networks[settings.network.id].name}.infura.io/v3/<your infura key>"ORG_ID = "test_org_id"SERVICE_ID = "test_service_id"'
                )
