...
class UploadService:
    ...
    def store_file(self, request: UploadFileRequest) -> dict[str, str]:
        file_extension = settings.files.FILE_EXTENSION[request.content_type]
        ...
        with open(temp_file_path, "wb") as file:
            file.write(request.raw_file_data)

        bucket = UPLOAD_TYPE_DETAILS[request.type]["bucket"]
        bucket_path: str = UPLOAD_TYPE_DETAILS[request.type]["bucket_path"]
        bucket_path_params = [date_time_for_filename(), file_extension]
        if request.type == UploadType.FEEDBACK.value:
            pass
        elif request.type == UploadType.ORG_ASSETS.value:
            bucket_path_params = [org_uuid] + bucket_path_params
        else:
