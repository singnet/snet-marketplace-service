...
class StubsGeneratorService:
    ...
    def generate_python_stubs(self, request: StubsGenerationRequest):
        ...
        boto_utils.download_folder_contents_from_s3(bucket = input_bucket, key = input_key,
                                                    target = tmp_paths["proto"])
        proto_location = None
        proto_names = []
        ...
        self._prepare_readme_file(target_path = os.path.join(tmp_paths["base"], "readme.txt"), service_id = service_id)

        # generate boilerplate stubs
        utils.copy_directory(tmp_paths["stubs"], tmp_paths["boilerplate"])
        self._prepare_boilerplate_template(org_id = org_id, service_id = service_id,
                                           target_location = tmp_paths["boilerplate"])

        if output_s3_path:
            utils.zip_file(source_path = Path(tmp_paths["base"]), zipped_path = Path(tmp_paths["result"]))
            boto_utils.s3_upload_file(filename = tmp_paths["result"], bucket = output_bucket,
                                      key = output_key + "python.zip")
    ...
    @staticmethod
    def _download_extract_and_upload_proto_files(filename, input_file_extension, bucket_name, download_file_path,
                                                 upload_file_path):
    ...
    def _prepare_boilerplate_template(self, target_location, org_id, service_id):
        for content in PYTHON_BOILERPLATE_TEMPLATE:
            context = self._prepare_data(PYTHON_BOILERPLATE_TEMPLATE[content]["content"], org_id, service_id)
            create_text_file(
