...
def daemon_health_check(daemon_endpoint: str) -> dict:
    try:
        ...
        logger.warning(f"Request exception, daemon endpoint {daemon_endpoint} is not available")
    except Exception:
        logger.warning("Error in invoking the daemon's health check function")
    return None