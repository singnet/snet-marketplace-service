...
class SignatureAuthenticator(object):
    ...
    def verify_current_block_number(self):
        ...
        print(f"current block {current_block_number}\nsigned clock number {signed_block_number}")
        if (
            current_block_number > signed_block_number + self.BLOCK_LIMIT
            or current_block_number < signed_block_number - self.BLOCK_LIMIT
        ):
            print("current_block_number is more than signed block limit %s", current_block_number)
            return False
        return True
...
class DaemonAuthenticator(SignatureAuthenticator):
    ...
    def get_public_keys(self):
        ...
        logger.info(
            f"Got stored daemon addresses {stored_public_keys} for {organization_id} {service_id} {group_id}"
        )
