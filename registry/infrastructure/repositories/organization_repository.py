...
from registry.domain.models.organization_member import (
...
class OrganizationPublisherRepository(BaseRepository):
    def get_organizations(
        self, status: str | None = None, limit: int | None = None, org_type: str | None = None
    ) -> List[OrganizationEntity]:
        organization_query = self.session.query(Organization).join(
            OrganizationState, Organization.uuid == OrganizationState.org_uuid
        )
        if status:
    ...
    def get_organization(
        ...
        if org_id:
            organization_query = organization_query.filter(
                text("org_id COLLATE utf8mb4_bin = :org_id")
    ...
    def get_all_orgs_for_user(
        self, username
    ) -> List[Tuple[OrganizationEntity, OrganizationMemberEntity]]:
    ...
    def store_metadata_uri(self, organization, username):
        organization_db = (
            self.session.query(Organization).filter(Organization.uuid == organization.uuid).first()
        )
    ...
    @BaseRepository.write_ops
    def persist_publish_org_transaction_hash(
        self, org_uuid, transaction_hash, wallet_address, nonce, username
    ):
        organization = (
            self.session.query(Organization).filter(Organization.uuid == org_uuid).first()
        )
    ...
    def store_organization(self, organization, username, state, test_transaction_hash=None):
        if state in [
            ...
            organization_db_model = (
                ...
                .filter(Organization.uuid == organization.uuid)
                .first()
            )

            if organization_db_model is None:
                self.add_organization(
                    organization, username, state, test_transaction_hash=test_transaction_hash
                )
            else:
    ...
    def add_organization(
        self, organization, username, state, address="", test_transaction_hash=None
    ):
    ...
    def _update_organization(
        ...
        self.session.query(Group).filter(Group.org_uuid == organization.uuid).delete(
    ...
    def get_org_member(
        self, username=None, org_uuid=None, role=None, status=None, invite_code=None
        ...
        if invite_code is not None:
            org_member_query = org_member_query.filter(
                OrganizationMember.invite_code == invite_code
            )
        ...
        return (
            OrganizationFactory.org_member_domain_from_repo_model_list(org_member)
            if org_member
            else []
        )
    ...
    def org_member_verify(self, username: str, invite_code: str) -> bool:
        org_members = (
            self.session.query(OrganizationMember)
    ...
    def delete_members(self, org_member_list, member_status):
        ...
        self.session.query(OrganizationMember).filter(
            OrganizationMember.username.in_(member_username_list)
    ...
    def update_org_member(self, username, wallet_address, invite_code):
        ...
        if (
            org_member.status == OrganizationMemberStatus.PUBLISHED.value
            and org_member.address == wallet_address
        ):
            org_member.username = username
        elif (
            org_member.status == OrganizationMemberStatus.PENDING.value
            and org_member.username == username
        ):
    ...
    def get_org_state_with_status(self, status: str) -> List[OrganizationStateEntity]:
        organization_state_db = (
            self.session.query(OrganizationState).filter(OrganizationState.state == status).all()
        )
    ...
    def update_org_status(self, org_uuid_list, prev_state, next_state):
        self.session.query(OrganizationState).filter(
            OrganizationState.org_uuid.in_(org_uuid_list)
        ).filter(OrganizationState.state == prev_state).update(
            {OrganizationState.state: next_state}, synchronize_session=False
        )
        self.session.commit()
    ...
    @BaseRepository.write_ops
    def delete_organization(self, org_uuid: str):
        self.session.query(Organization).filter(Organization.uuid == org_uuid).delete()
        self.session.commit()
