...
class TestOrganizationPublisherService(unittest.TestCase):
    ...
    def test_add_organization_draft(self, mock_boto, mock_ipfs):
        ...
        self.assertDictEqual(
            org_dict,
            {'name': 'dummy_org', 'org_id': test_org_uuid, 'org_uuid': test_org_uuid,
             'org_type': 'individual', 'description': 'this is the dummy org for testcases',
             'short_description': 'this is the short description',
             'url': 'https://dummy.dummy', 'duns_no': '123456789',
             'origin': 'PUBLISHER_DAPP', 'contacts': [],
             'assets': {'hero_image': {'url': ""}},
             'metadata_ipfs_hash': '1234',
             'groups': [
                 {'name': 'my-group', 'id': 'IN2fDELdv5zKzgKgFpXVBvNKBhbIc0zYoA750wJ/s60=', 'payment_address': '0x123',
                  'payment_config': {'payment_channel_storage_type': 'etcd', 'payment_expiration_threshold': 40320,
                                     'payment_channel_storage_client': {'endpoints': ['http://127.0.0.1:2379'],
                                                                        'request_timeout': '3s',
                                                                        'connection_timeout': '5s'}}},
                 {'name': 'group-123', 'id': 'S2fjwWLCjHsoheHagBIQ6893ipA8AV97hdAdqnrT/Fk=', 'payment_address': '0x123',
                  'payment_config': {'payment_channel_storage_type': 'etcd', 'payment_expiration_threshold': 40320,
                                     'payment_channel_storage_client': {'endpoints': ['http://127.0.0.1:2379'],
                                                                        'request_timeout': '3s',
                                                                        'connection_timeout': '5s'}}}],
             'addresses': [
                 {'address_type': 'headquater_address', 'street_address': 'F102', 'apartment': 'ABC Apartment',
                  'city': 'TestCity', 'pincode': '123456', 'state': "state", 'country': 'TestCountry'},
                 {'address_type': 'mailing_address', 'street_address': 'F102', 'apartment': 'ABC Apartment',
                  'city': 'TestCity', 'pincode': '123456', 'state': "state", 'country': 'TestCountry'}],
             'state': {}})
        payload = {
        ...
        if not validate_dict(payload, ORG_PAYLOAD_REQUIRED_KEYS):
            raise Exception("Testcase payload is invalid")
        OrganizationPublisherService(None, username).add_organization_draft(payload)
        org_db_model = org_repo.session.query(Organization).filter(Organization.uuid == test_org_uuid).all()
        if len(org_db_model) != 1:
            assert False
        organization = OrganizationFactory.org_domain_entity_from_repo_model(org_db_model[0])
        ...
        self.assertDictEqual(
            org_dict,
            {'name': 'dummy_org', 'org_id': test_org_uuid, 'org_uuid': test_org_uuid,
             'org_type': 'individual', 'description': 'that is the dummy org for testcases',
             'short_description': 'that is the short description',
             'url': 'https://dummy.dummy', 'duns_no': '123456789',
             'origin': 'PUBLISHER_DAPP', 'contacts': [],
             'assets': {'hero_image': {'url': ""}},
                 ...
                 {'name': 'my-group', 'id': 'IN2fDELdv5zKzgKgFpXVBvNKBhbIc0zYoA750wJ/s60=', 'payment_address': '0x123',
                  'payment_config': {'payment_channel_storage_type': 'etcd', 'payment_expiration_threshold': 40320,
                                     'payment_channel_storage_client': {'endpoints': ['http://127.0.0.1:2379'],
                                                                        'request_timeout': '3s',
                                                                        'connection_timeout': '5s'}}},
                 {'name': 'group-234', 'id': 'S2fjwWLCjHsoheHagBIQ6893ipA8AV97hdAdqnrT/Fk=', 'payment_address': '0x123',
                  'payment_config': {'payment_channel_storage_type': 'etcd', 'payment_expiration_threshold': 40320,
                                     'payment_channel_storage_client': {'endpoints': ['http://127.0.0.1:2379'],
                                                                        'request_timeout': '3s',
                                                                        'connection_timeout': '5s'}}}],
    ...
    def test_submit_org_for_approval(self, mock_boto_utils, mock_ipfs_utils):
        ...
        test_org_uuid = organization.uuid
        org_dict = organization.to_dict()
        self.assertEqual(org_dict["state"]["state"], OrganizationStatus.APPROVAL_PENDING.value)
        org_dict["state"] = {}
        org_dict["assets"]["hero_image"]["url"] = ""
        self.assertDictEqual(
            org_dict,
