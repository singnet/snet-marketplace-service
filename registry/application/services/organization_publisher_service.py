...
from registry.application.access import OrganizationAccessControl
...
class OrganizationService:
    ...
    def add_organization_draft(self, payload, username):
        ...
        if self.is_on_boarding_approved(username):
            org_repo.move_non_published_org_to_history(organization.org_uuid)
            org_repo.add_org_with_status(organization, OrganizationStatus.APPROVED.value, username)
        else:
            org_repo.draft_update_org(organization, username)
            org_repo.move_non_published_org_to_history(organization.org_uuid)
    ...
    def is_on_boarding_approved(self, username):

        published_orgs = org_repo.get_published_org_for_user(username)
    ...
    def publish_org_to_ipfs(self, username):
    ...
    def save_transaction_hash_for_publish_org(self, transaction_hash, wallet_address, username):
        orgs = org_repo.get_approved_org(self.org_uuid)
        if len(orgs) == 0:
        ...
        org_repo.add_org_with_status(organization, OrganizationStatus.PUBLISH_IN_PROGRESS.value, username,
                                     transaction_hash=transaction_hash, wallet_address=wallet_address)
        return "OK"

    def get_organizations_for_user(self, username):
        organizations = org_repo.get_latest_organization(username)
        response = []
    ...
    def add_group(self, payload, username):
        ...
        org_repo.add_group(groups, self.org_uuid, username)
        return "OK"
...
if __name__ == '__main__':
    OrganizationService("").get_role_for_org_member()
