...
class OrganizationTransactionStatus:
    ...
    def update_transaction_status(self):
        org_transaction_data = org_repo.get_org_state_with_status(OrganizationStatus.PUBLISH_IN_PROGRESS.value)
        failed_org_transactions = []
        for org_state in org_transaction_data:
            org_uuid = org_state.org_uuid
            ...
            if transaction_receipt is None:
                logger.error(f"Failed to get the transaction from blockchain for "
                             f"org:{org_uuid} hash:{transaction_hash}")
            else:
                if transaction_receipt.status == 0:
                    failed_org_transactions.append(org_uuid)
                    logger.info(f"Failed service_uuid:{org_uuid} transaction_hash:{transaction_hash}")
        if len(failed_org_transactions) == 0:
            return
        org_repo.update_org_status(failed_org_transactions, OrganizationStatus.PUBLISH_IN_PROGRESS.value,
