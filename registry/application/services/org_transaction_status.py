...
class OrganizationTransactionStatus:
    ...
    def update_transaction_status(self):
        org_transaction_data = self.org_repo.get_org_state_with_status(
        ...
        for org_state in org_transaction_data:
            ...
            if transaction_receipt is None:
                logger.error(
                    f"Failed to get the transaction from blockchain for "
                ...
                if transaction_receipt.status == 0:
                    ...
                    logger.info(
                        f"Failed service_uuid:{org_state.org_uuid} transaction_hash:{transaction_hash}"
                    )
