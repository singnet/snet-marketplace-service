...
class FileService:
    def delete(self, file_details):
        ...
        unsuccessful_files = [
            s3_file_details.get("Key", "") for s3_file_details in unsuccessful_deletes
        ]
        response = {
    ...
    def get_s3_bucket_and_prefix(self, file_details):
        ...
        if file_type == FileType.ORG_ASSETS.value["file_type"] and "org_uuid" in file_details:
            ...
            file_type == FileType.SERVICE_ASSETS.value["file_type"]
            and "org_uuid" in file_details
            and "service_uuid" in file_details
        ):
            org_uuid = file_details["org_uuid"]
            service_uuid = file_details["service_uuid"]
            bucket = FileType.SERVICE_ASSETS.value["bucket"]
            prefix = FileType.SERVICE_ASSETS.value["bucket_path"].format(
                org_uuid=org_uuid, service_uuid=service_uuid
            )

        elif (
            file_type == FileType.SERVICE_PROTO_FILES.value["file_type"]
            and "org_uuid" in file_details
            and "service_uuid" in file_details
        ):
            org_uuid = file_details["org_uuid"]
            ...
            prefix = FileType.SERVICE_PROTO_FILES.value["bucket_path"].format(
                org_uuid=org_uuid, service_uuid=service_uuid
            )

        elif (
            file_type == FileType.SERVICE_PAGE_COMPONENTS.value["file_type"]
            and "org_uuid" in file_details
            and "service_uuid" in file_details
        ):
            org_uuid = file_details["org_uuid"]
            ...
            prefix = FileType.SERVICE_PAGE_COMPONENTS.value["bucket_path"].format(
                org_uuid=org_uuid, service_uuid=service_uuid
            )

        elif (
            file_type == FileType.SERVICE_GALLERY_IMAGES.value["file_type"]
            and "org_uuid" in file_details
            and "service_uuid" in file_details
        ):
            org_uuid = file_details["org_uuid"]
            ...
            prefix = FileType.SERVICE_GALLERY_IMAGES.value["bucket_path"].format(
                org_uuid=org_uuid, service_uuid=service_uuid
            )
        else:
            raise InvalidFileTypeException()
