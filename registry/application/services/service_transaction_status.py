    ...
    def update_transaction_status(self):
        ...
        for state in states:
            try:
                ...
                logger.info(f"Transaction not found service_uuid:{state.service_uuid} transaction_hash:{state.transaction_hash}")
                failed_service_transactions.append(state.service_uuid)
                continue

            if transaction_receipt is None or transaction_receipt.status == 0:
                failed_service_transactions.append(state.service_uuid)
                logger.info(f"Failed service_uuid:{state.service_uuid} transaction_hash:{state.transaction_hash}")
