...
class BlockChainUtil(object):
    ...
    def __init__(self, ws_provider):
        ...
        self.web3_object = Web3(
            web3.providers.WebsocketProvider(self.provider))
    ...
    def __init__(self, type, provider):
        if type == "HTTP_PROVIDER":
            ...
            raise Exception(
                "Only HTTP_PROVIDER and WS_PROVIDER type are supported.")
    ...
    def generate_signature(self, data_types, values, signer_key):
        signer_key = "0x" + \
            signer_key if not signer_key.startswith("0x") else signer_key
