...
class PaypalPayment(Payment):
    ...
    def initiate_payment(self, order_id, item_details):
        paypal_payload = self.get_paypal_payload(
            order_id, item_details["org_id"], item_details["service_id"]
        )
    ...
    def get_paypal_payload(self, order_id, org_id, service_id):
        paypal_payload = {
            ...
            "redirect_urls": {
                "return_url": PAYMENT_RETURN_URL.format(
                    org_id, service_id, order_id, self._payment_id
                ),
                "cancel_url": PAYMENT_CANCEL_URL.format(
                    org_id, service_id, order_id, self._payment_id
                ),
            },
            "transactions": [
