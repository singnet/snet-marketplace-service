...
class HandleContracts:
    ...
    def __handle_events(self, evt_data):
        ...
        if event in MPE_EVTS:
            ...
            if event in ['ServiceCreated', 'ServiceMetadataModified']:
    ...
    def _process_events(self, events):
        #type is extra paraeter here fix it with event name


        for event in events:
            type = self.get_type_from_event_name() #type can be MPE or REGISTRY
            try:
                print('process_events::row_id: ',
                      event['row_id'], '|', type, '|', event['block_no'])
                handled = self.__handle_events(
                    json.loads(event['json_str']))
                if handled:
                    self.db_obj.updt_raw_evts(
                        event['row_id'], type, err_cd=0, err_msg='')
            except Exception as e:
                err_msg = self.err_obj.log_err_msg(
                    err=e, fn_nme='_process_events')
                self.util_obj.report_slack(
                    type=1, slack_msg=err_msg, SLACK_HOOK=SLACK_HOOK)
                print('writing error for row_id: ', event['row_id'])
                self.db_obj.updt_raw_evts(
                    event['row_id'], type, err_cd=1, err_msg=err_msg)

    def handle_contract(self,events):
        try:
            self._init_w3()
