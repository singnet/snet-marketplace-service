...
class OrderService:
    ...
    def initiate_order(self, user_data, payload_dict):
        ...
        try:
            redirect_response = self.manage_initiate_payment(order_id=order_id, amount=amount, currency=currency,
    ...
    def execute_order(self, user_data, payload_dict):
        ...
        payment_executed = self.manage_execute_payment(order_id=order_id, payment_id=payment_id,
                                                       payment_details=payment_details,
                                                       payment_method=payment_method)
        status = Status.PAYMENT_EXECUTED
        try:
            status = Status.ORDER_PROCESSING_FAILED
            data = self.manage_process_order(order_id=order_id, order_type=order_type, amount=payload_dict["amount"],
                                             currency=payload_dict["currency"])
    ...
    def manage_initiate_payment(self, username, order_id, amount, currency, payment_method):
        obj_dapp_order_manager = DappOrderManager()
        initiate_payment_data = obj_dapp_order_manager.initiate_payment(order_id=order_id, amount=amount,
                                                                        currency=currency,
                                                                        payment_method=payment_method)
    ...
    def manage_create_order(self, username, item_details, amount, currency):
        obj_dapp_order_manager = DappOrderManager()
        order_details = obj_dapp_order_manager.create_order(username=username, item_details=item_details, amount=amount,
                                                            currency=currency)
        if ["status"] == 200:
    ...
    def manage_execute_payment(self, username, order_id, payment_id, payment_details, payment_method):
        ...
        payment_executed = obj_dapp_order_manager.execute_payment_for_order(order_id=order_id, payment_id=payment_id,
