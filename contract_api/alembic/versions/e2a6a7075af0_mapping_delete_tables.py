...
SET description = 
    CASE 
        WHEN RIGHT(description, 2) <> '"}' THEN 
            CONCAT(
                LEFT(description, LENGTH(description) - 2),
                '"}'
            )
        ELSE 
            description
    END
WHERE description IS NOT NULL AND LENGTH(description) > 2;
""")
    ...
    op.create_foreign_key(None, 'org_group', 'organization', ['org_id'], ['org_id'], onupdate = 'CASCADE',
                          ondelete = 'CASCADE')
    ...
    op.create_foreign_key(None, 'service', 'organization', ['org_id'], ['org_id'], onupdate = 'CASCADE',
                          ondelete = 'CASCADE')
    op.drop_column('service', 'row_updated')
    op.drop_column('service', 'row_created')
    op.add_column('service_endpoint',
                  sa.Column('created_on', mysql.TIMESTAMP(), server_default = sa.text('now()'), nullable = False))
    op.add_column('service_endpoint',
                  sa.Column('updated_on', mysql.TIMESTAMP(), server_default = sa.text('now()'), nullable = False))
    ...
    op.create_foreign_key(None, 'service_endpoint', 'service', ['service_row_id'], ['row_id'], onupdate = 'CASCADE',
                          ondelete = 'CASCADE')
    op.drop_column('service_endpoint', 'row_updated')
    op.drop_column('service_endpoint', 'row_created')
    op.add_column('service_group',
                  sa.Column('created_on', mysql.TIMESTAMP(), server_default = sa.text('now()'), nullable = False))
    op.add_column('service_group',
                  sa.Column('updated_on', mysql.TIMESTAMP(), server_default = sa.text('now()'), nullable = False))
    ...
    op.create_foreign_key(None, 'service_group', 'service', ['service_row_id'], ['row_id'], onupdate = 'CASCADE',
                          ondelete = 'CASCADE')
