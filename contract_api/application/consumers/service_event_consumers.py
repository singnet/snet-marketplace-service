...
from contract_api.infrastructure.db import session_scope, DefaultSessionFactory
...
class ServiceCreatedEventConsumer(EventConsumer):
    def __init__(self):
        super().__init__()
    ...
    def on_event(self, request: RegistryEventConsumerRequest) -> None:
        ...
        mpe_contract_path = os.path.abspath(
            f"{CONTRACT_BASE_PATH}/node_modules/singularitynet-platform-contracts/networks/MultiPartyEscrow.json"
        )
        ...
        if service_mpe_address != current_mpe_address:
            with session_scope(self._session_factory) as session:
                ...
                try:
                    self._service_repository.delete_service(session, org_id, service_id)
                except Exception:
                    logger.info(
                        f"No service found with org_id {org_id} and service_id {service_id}"
                    )

                org_data = self._organization_repository.get_organization(session, org_id)
                if org_data is None:
                    logger.info(f"No organization found with org_id {org_id}")
                    return
                org_services = self._service_repository.get_services(session, org_id)
            ...
            logger.info(
                f"Service's MPE address is the same as the current MPE address. Checking organization {org_id}."
            )
            with session_scope(self._session_factory) as session:
                logger.info(
                    f"Processing service creation for org_id: {org_id}, service_id: {service_id}"
                )

                org_data = self._organization_repository.get_organization(session, org_id)

                logger.info(f"Retrived organiation data: {org_data}")
    ...
    def _process_service_data(
        self, org_id: str, service_id: str, new_hash: str, new_service_metadata: dict
    ) -> None:
        with session_scope(self._session_factory) as session:
            existing_service_metadata = self._service_repository.get_service_metadata(
                session, org_id, service_id
            )
            logger.info(
                f"Existing service metadata: {existing_service_metadata.to_response() if existing_service_metadata else None}"
            )

            assets_url = self._get_new_assets_url(
                org_id, service_id, new_service_metadata, existing_service_metadata
            )
            logger.info(f"New assets url :: {assets_url}")

            self._service_repository.delete_service_dependents(
                session=session, org_id=org_id, service_id=service_id
            )

            service_data = self._service_repository.upsert_service(
                session,
                NewServiceDomain(
                    org_id=org_id, service_id=service_id, hash_uri=new_hash, is_curated=True
                ),
            )
            ...
            self._create_service_media(
                session=session, service=service_data, service_media=service_media
            )
...
class ServiceCreatedDeploymentEventHandler(EventConsumer):
    def __init__(self):
        super().__init__()
        self._service_repository = NewServiceRepository()
        self._organization_repository = NewOrganizationRepository()
    ...
    def on_event(
        self, request: RegistryEventConsumerRequest, session: Session | None = None
    ) -> None:  # type: ignore
        org_id = request.org_id
        service_id = request.service_id
        ...
        if session is None:
            with session_scope(self._session_factory) as scoped_session:
                self._handle_event(scoped_session, org_id, service_id)
        else:
            self._handle_event(session, org_id, service_id)

    def _handle_event(self, session: Session, org_id: str, service_id: str) -> None:
        service_metadata = self._service_repository.get_service_metadata(
            session, org_id, service_id
        )
        if service_metadata is not None:
            self.process_service_deployment(session, service_metadata)

    @staticmethod
    def _extract_zip_and_and_tar(org_id: str, service_id: str, s3_url: str) -> str:
        ...
        extracted_file_path = (
            extracted_zip_directory + "/" + zip_file_name.split(".")[0].split("_")[1]
        )
    ...
    def _get_s3_path_url_for_proto_and_component(self, org_id, service_id) -> tuple[str, str]:
        proto_file_s3_path = ""
        component_files_s3_path = ""
        lambda_payload = {
            "httpMethod": "GET",
            "queryStringParameters": {"org_id": org_id, "service_id": service_id},
        }
        ...
        if response["statusCode"] == 200:
            ...
            for stub_object in stub_objects:
                generated_stubs_url.append(
                    f"https://{output_bucket}.s3.{REGION_NAME}.amazonaws.com/{stub_object['Key']}"
                )
    ...
    def update_proto_stubs(
        self, session: Session, service_metadata: ServiceMetadataDomain, proto_stubs: list[str]
    ) -> None:
        for stub in proto_stubs:
            # filename, _ = utils.get_file_name_and_extension_from_path(path=stub)
            filename = "dadadadad"
            self._service_repository.upsert_service_media(
                session,
                NewServiceMediaDomain(
    ...
    def upload_proto_file_from_hash_to_bucket(self, org_id, service_id, asset_hash):
        ...
        self._s3_util.push_file_to_s3(
            temp_output_path,
            ASSETS_COMPONENT_BUCKET_NAME,
            f"assets/{org_id}/{service_id}/proto.tar.gz",
        )

    def process_service_deployment(
        self, session: Session, service_metadata: ServiceMetadataDomain
    ) -> None:
        org_id = service_metadata.org_id
        service_id = service_metadata.service_id
        model_hash = service_metadata.model_hash
        logger.info(f"Processing Service deployment for {org_id} {service_id}")
        self.upload_proto_file_from_hash_to_bucket(
            org_id=org_id, service_id=service_id, asset_hash=model_hash
        )
        proto_stubs = self.compile_proto_stubs(org_id=org_id, service_id=service_id)
        self.update_proto_stubs(
            session=session, service_metadata=service_metadata, proto_stubs=proto_stubs
        )
